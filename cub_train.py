...
def get_config():
    config = {
        ...
        "N_list": [2, 3, 2],
        "channels": 512,
        "init_lr": 0.0005,   #todo should be 0.0001
        "weight_decay": 1e-4,
        "num_parts": 3,
        "m": [0.5],
    }
...
def main(config, bit,margin):
    ...
    state_dict = torch.load('checkpoint/CUB_model_ft_190.t', map_location=device)['model_state_dict']
    ...
    #optimizer = optim.RMSprop(parameters, lr=config["init_lr"], weight_decay=config["weight_decay"],lr_type="step")
    ...
    log = open("d:/lixue/fine-grained/code/MMAL+sRLH/logcub.txt", mode = "a+", encoding = "utf-8")
    for epoch in range(config["epoch"]):
        ...
        #print('epoch:{},bit:{},margin:{},loss:{:.4f},L_g1:{:.4f},L_g2:{:.4f},L_g3:{:.4f},L_h2:{:.4f}'.format(epoch+1,bit,margin, loss,L_g1,L_g2,L_g3,L_h2),file=log)
        print('epoch:{},bit:{},margin:{},loss:{:.4f},L_g:{:.4f},L_h:{:.4f}'.format(epoch+1,bit,margin, loss,L_g,L_h))
        loss = loss.cpu().detach().reshape((1)).numpy()
        if not os.path.isdir('loss_result/'+config["dataset"]+config["info"]+'/'+str(bit)):
            os.makedirs('loss_result//'+config["dataset"]+config["info"]+'//'+str(bit))
        with open('loss_result/'+config["dataset"]+config["info"]+'/'+str(bit)+'/'+'loss.txt','a+') as f:
        ...
        if (epoch + 1) % config["test_map"] == 0:
            ...
            print('epoch:{},time:{},mAP:{:.4f},Best_mAP:{:.4f}'.format(epoch, current_time, mAP, Best_mAP))
            print(str(config["info"])+'bit:{},epoch:{},time:{},mAP:{:.4f},Best_mAP:{:.4f}'.format(bit,epoch, current_time, mAP, Best_mAP),file=log)
    log.close()
...
if __name__ == '__main__':
    ...
    #logger.add('logs/cub_%s_{time}.log' %str(config["dataset"])[20:-2])
