...
def main(config, bit,margin):
    ...
    state_dict = torch.load('checkpoint/CAR_model_ft_90.t', map_location=device)['model_state_dict']
    ...
    log = open("d:/lixue/fine-grained/code/MMAL+sRLH/logcar.txt", mode = "a+", encoding = "utf-8")
...
if __name__ == '__main__':
    config = get_config()
    print(config)
    #logger.add('logs/car_%s_{time}.log' %str(config["dataset"])[20:-2])
